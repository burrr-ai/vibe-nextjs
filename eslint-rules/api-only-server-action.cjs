...
module.exports = {
  meta: {
    ...
    messages: {
      noRouteHandler:
        'Route handlers (app/api/*) are not allowed. Use server actions in @/api instead. Exception: app/api/auth/* for authentication. (see src/services/api.ai.md)',
      noFetchInClient:
        'Direct fetch() calls in client components are not allowed. Use server actions from @/api through @/state instead. (see src/services/api.ai.md)',
    },
    schema: [],
  },
